...
namespace HPI.WebAPI.Controllers
{
    public class ProductController : ApiController
    {
        ...
        {
            List<Rule> prodRules = GetValidationRules(productModel);
            RuleEngine engine = new RuleEngine();
            var childPropCheck = engine.CompileRules<ProductModel>(prodRules);
            if(prodRules.Count == 2 && childPropCheck(productModel))
            {
                ...
                return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "Product failed to meet the validation criteria");
        }
